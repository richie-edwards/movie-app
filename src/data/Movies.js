...
const _between = (value, from, to) => {
  console.log(
    "between",
    Number(value) >= Number(from) && Number(value) <= Number(to)
  );
...
const movie = {
  ...
  // genres would just be coming from a database lookup table
  genres: () => {
    return [
  ...
  getMovies: ({ genre, yearRange, ratingRange }) => {
    console.log(genre, yearRange, ratingRange);
    return MOVIES.filter((movie) => {
      return (
        movie.genres.includes(genre) &&
        _between(movie.releaseYear, yearRange[0], yearRange[1]) &&
        _between(movie.rating, ratingRange[0], ratingRange[1])
      );
    });
  },
};
